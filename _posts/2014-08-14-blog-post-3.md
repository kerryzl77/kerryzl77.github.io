...
<figure>
  <img src="{{ '/images/iterativeforecast.png' | relative_url }}" alt="DeepAR ancestral sampling drawing multiple iterative forecast paths">
</figure>
...
Here, $$\mathbf{x}_{1:L}$$ are the past observations (plus any covariates), and **mask** is used to prevent the model from peeking at future time steps in self-attention (e.g., in language modeling). For direct forecasting, the model outputs $$\hat{\mathbf{y}}_{1:H}$$ in a single forward pass.
