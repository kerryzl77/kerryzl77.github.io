...
*Here,* $\mathbf{x}_{1:L}$ are the past observations (plus any covariates), *and* $\mathbf{mask}$ is used to prevent the model from peeking at future time steps in self-attention (e.g., in language modeling). For direct forecasting, the *model outputs*  $\hat{y}_{1:H}$ in a single forward pass.
---
